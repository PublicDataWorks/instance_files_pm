<div>
  ...
  {{#each accusedOfficers}}
    ...
    GCPD policy, rules, or procedure when:
    <ol>
      {{#each allegations}}
  ...
  {{#each accusedOfficers}}
    {{#if letterOfficer}}
      {{#if (isEqual fullName "Unknown Officer")}}
        <p>
          The PM is unable to review <strong>{{fullName~}}’s</strong> disciplinary history as they are unable to be
      ...
      {{#if (isEqual letterOfficer.officerHistoryOptionId 1)}}
        <p>
          The PM reviewed <strong>{{rank}} {{fullName~}}’s</strong> history for the last five years and has determined
      ...
      {{#if (isEqual letterOfficer.officerHistoryOptionId 3)}}
        <p>
          The PM was unable to review <strong>{{rank}} {{fullName~}}’s</strong> history for the last five years
          because there was no officer
          disciplinary history available in the GCPD IAPro.
        </p>
        <br />
      {{/if}}
      {{#if (isEqual letterOfficer.officerHistoryOptionId 4)}}
        <p>
          The PM has reviewed <strong>{{rank}} {{fullName~}}'s</strong> disciplinary history for the last five years
  ...
  {{#if referralLetter.includeRetaliationConcerns}}
    ...
    <p><br></p>
    <p>Based on the information provided by the complainant, the PM is concerned about retaliation against the
      complainant.
