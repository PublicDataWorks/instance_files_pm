...
resource "aws_s3_bucket" "exports_bucket" {
  bucket = "${var.organization_name}-exports-${var.env_name}"
  acl = "private"
...
resource "aws_iam_policy" "env_policy" {
  name = "${var.organization_name}-${var.env_name}-bucket-access"
...
resource "aws_apigatewayv2_api" "api_gateway" {
  name          = "${var.organization_name}-${var.env_name}-http-api"
  protocol_type = "HTTP"
